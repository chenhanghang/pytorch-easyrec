...
class YoutubeSBC(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.user_mlp = MLP_MUL(self.user_dims, output_layer=False, **user_params)
        self.item_mlp = MLP_MUL(self.item_dims, output_layer=False, **item_params)
        self.mode = None

        # in-batch sampling index 第0维和第1维的索引
        self.index0 = np.repeat(np.arange(batch_size), n_neg + 1) #[b*(n_neg + 1)]
        self.index1 = np.concatenate([np.arange(i, i + n_neg + 1) for i in range(batch_size)]) # [b*(n_neg + 1)]
