...
class MMOE(nn.Module):
    ...
    def __init__(self, features, task_types, n_expert, expert_params, tower_params_list):
        ...
        self.experts = nn.ModuleList(
            MLP_MUL(self.input_dims, output_layer=False, **expert_params) for i in range(self.n_expert))
        self.gates = nn.ModuleList(
            MLP_MUL(self.input_dims, output_layer=False, **{
                "dims": [self.n_expert],
                "activation": "softmax"
            }) for i in range(self.n_task))  #n_gate = n_task
        self.towers = nn.ModuleList(MLP_MUL(expert_params["dims"][-1], **tower_params_list[i]) for i in range(self.n_task))
